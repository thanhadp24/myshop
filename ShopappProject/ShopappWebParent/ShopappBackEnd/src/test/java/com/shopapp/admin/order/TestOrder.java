...
public class TestOrder {
	...
	@Test
	public void testFindOrderByDateBetween() throws ParseException {
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		Date start = df.parse("2025-01-10");
		Date end = df.parse("2025-01-18");
		
		var orders = orderRepository.findByOrderTimeBetween(start, end);
		
		for(Order order: orders) {
			System.out.printf("%s | %s | %.2f | %.2f | %.2f", order.getId(), order.getOrderTime(),
